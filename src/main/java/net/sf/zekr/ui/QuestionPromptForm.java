...
public class QuestionPromptForm extends BaseForm {
	...
	public void init() {
		...
		if (hasProgress) {
			shell.addListener(CUSTOM_ZEKR_EVENT, new Listener() {
				public void handleEvent(Event event) {
					if (IMPORT_PROGRESS.equals(event.data)) {
						int p = Math.max(Math.min(event.detail, 100), 0);
						progessBar.setSelection(p);
					} else if (IMPORT_PROGRESS_DONE.equals(event.data)
							|| IMPORT_PROGRESS_FAILED.equals(event.data)) {
						
						if (IMPORT_PROGRESS_DONE.equals(event.data)) {
